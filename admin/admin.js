...
async function sendAudit(action, email, session) {
...
async function checkExistingSession() {
  ...
  if (lastActivity && Date.now() - lastActivity >= SESSION_TIMEOUT_MS) {
    ...
    await sendAudit('admin.logout.timeout', '', data.session);
...
loginForm.addEventListener('submit', async function(event) {
  ...
  try {
    ...
    if (error || !data || !data.session) {
      ...
      await sendAudit('admin.login.failed', email, null);
    ...
    await sendAudit('admin.login.success', email, data.session);
